...
The system SHALL render a Leaflet-based political map of India containing all 28 states and 8 union territories as individually interactive GeoJSON polygon features. The map SHALL use boundaries sourced from a bundled India states GeoJSON file (open-data, CC0 license). The previous SVG/`<path>`-based implementation is removed.
...
**Reason**: The SVG + `statePaths.ts` approach is replaced by the Leaflet GeoJSON layer. Removing it eliminates messy hand-extracted borders and enables proper geographic positioning of island territories.
**Migration**: Delete `src/map/statePaths.ts`. The `IndiaMap.tsx` SVG component is replaced by `WorldMap.tsx`.

### Requirement: SmallUTChips sidebar
**Reason**: Island UTs (Andaman & Nicobar, Lakshadweep) now render at their correct geographic positions on the Leaflet map. The sidebar chip workaround is no longer needed.
**Migration**: Delete `src/components/map/SmallUTChips.tsx`. Remove its usage from `WorldMap.tsx`.
